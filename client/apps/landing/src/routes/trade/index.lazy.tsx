  ...
  return (
    <>
      ...
      <ScrollHeader className="flex flex-row justify-between items-center" onScrollChange={setIsHeaderScrolled}>
        {/* Filter UI */}
        {isHeaderScrolled ? <h4 className="text-lg sm:text-xl font-bold mb-2 pl-4">{"Season 1 Pass"}</h4> : <div></div>}
        ...
        <div className="flex flex-col gap-2 px-2">
          ...
          >
            ...
            {filteredSeasonPasses.length === 0 && Object.keys(selectedFilters).length > 0 && (
              <div className="text-center py-6 text-muted-foreground">No Season Passes match the selected filters.</div>
            )}
            {totalPasses === 0 && (
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:block font-semibold">
                ...
                <span className="text-xs text-muted-foreground">
                  (Max Price:{" "}
                  {selectedPasses.length > 0
                    ? formatUnits(BigInt(Math.max(...selectedPasses.map((pass) => Number(pass.best_price_hex)))), 18)
                    : 0}
                  )
                </span>
              </div>
